...
export function getCatalogCategories(loadImages = true): CatalogCategory[] { if (!fs.existsSync(assetsDirectory)) return []; return fs.readdirSync(assetsDirectory, { withFileTypes: true }).filter((entry) => entry.isDirectory()).sort((a, b) => a.name.localeCompare(b.name)).map((entry) => { const file = loadImages ? fs.readdirSync(path.join(assetsDirectory, entry.name)).find((item) => /\.(jpe?g|png|webp)$/i.test(item)) : undefined; const category = label(entry.name); return { id: slug(category), name: category, category, image: file ? assetPath(entry.name, file) : "" }; }); }
...
export default async function ProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <PageFrame><section className="product-detail-page"><Link className="back-link" href="/products">&lt;- Back to Products</Link><div className="detail-main"><ProductGallery images={[product.image]} name={product.name} /><div className="detail-copy"><h1>{product.name}</h1><div className="rating">★★★★★ <small>{product.category}</small></div><div className="detail-price"><b>LKR {product.price.toLocaleString("en-LK")}</b></div><p>A thoughtfully selected {product.category.toLowerCase()} piece from Patio King, made to bring comfort, character, and lasting style to modern interiors.</p><div className="detail-actions"><a className="buy" href={inquiryHref} target="_blank" rel="noreferrer">Inquire Now -&gt;</a></div><small>Category: {product.category}</small><small>Details: {product.category}, modern furniture, home furniture.</small></div></div><ProductTabs name={product.name} category={product.category} products={relatedProducts} /></section></PageFrame>;
}
