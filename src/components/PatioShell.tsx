...
export function PatioHeader() {
  ...
  return <header className="reference-header"><Link className="brand-mark" href="/" onClick={closeMenu}><strong>PK</strong><small>PATIO KING</small></Link><button className={`mobile-menu-toggle ${menuOpen ? "open" : ""}`} type="button" aria-label={menuOpen ? "Close navigation" : "Open navigation"} aria-expanded={menuOpen} onClick={() => setMenuOpen((open) => !open)}><span /><span /><span /></button><nav className={menuOpen ? "open" : ""}><Link href="/" onClick={closeMenu}>Home</Link><Link href="/products" onClick={closeMenu}>Products</Link><Link href="/customize" onClick={closeMenu}>Customize</Link><Link href="/appointment" onClick={closeMenu}>Book a Visit</Link><Link href="/projects" onClick={closeMenu}>Projects</Link><Link href="/about" onClick={closeMenu}>About</Link><Link href="/contact" onClick={closeMenu}>Contact</Link></nav><div className="head-icons"><Link className="header-icon profile-icon" href="/profile" aria-label="Profile" title="Profile">♙</Link></div></header>;
}
